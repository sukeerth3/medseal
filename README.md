...
- `terraform` 1.5+
- `java` 17 and `mvn`
- `python3.11` or another Python 3.11+ executable
- `node` 20+ and `npm`
- `docker`
- `git`, `patch`, `curl`, `ssh`, and `scp`
- `nitro-cli` on the Nitro-capable EC2 host that builds or measures the EIF

macOS local validation host:

```bash
brew install awscli terraform openjdk@17 maven python@3.11 node@20 git patch curl
brew install --cask docker

export PATH="$(brew --prefix openjdk@17)/bin:$PATH"
```

Amazon Linux 2023 Nitro build host:

```bash
sudo dnf install -y yum-utils git patch unzip curl openssh-clients docker \
  maven java-17-amazon-corretto-devel python3.11 python3.11-pip \
  awscli aws-nitro-enclaves-cli aws-nitro-enclaves-cli-devel
...
Log out and back in after the `usermod` commands so the Docker and Nitro group memberships apply.
...
patch --version
ssh -V
nitro-cli --version   # Nitro build host only
```

### 2. Configure AWS Access

```bash
aws configure --profile medseal
AWS_PROFILE=medseal AWS_REGION=us-east-1 aws sts get-caller-identity
```

The AWS principal needs permission to create VPC, EC2, IAM, KMS, S3, DynamoDB, and CloudWatch resources. You also need an EC2 key pair, an allowed ingress CIDR, and client IAM principal ARNs that may call KMS directly for `GenerateDataKey`.
...
python -m pip install -r enclave/requirements.txt

npm --prefix client ci
```

Maven resolves gateway dependencies during `mvn test` or `mvn package`. Terraform downloads providers during `terraform init`.

### 4. Build Runtime Artifacts

```bash
# Gateway jar: gateway/target/gateway-1.0.0.jar
mvn -f gateway/pom.xml -B package -DskipTests

# Enclave support binaries:
# enclave/bin/kmstool_enclave_cli and enclave/bin/libnsm.so
./scripts/build-kmstool.sh

# Nitro Enclave Image File and manifest:
# build/medseal.eif and build/manifest.json
./scripts/build-enclave.sh
./scripts/extract-pcrs.sh build/medseal.eif > infrastructure/pcrs.auto.tfvars
```

`scripts/build-enclave.sh` calls `scripts/build-kmstool.sh` automatically if the enclave support binaries are missing. The EIF build and PCR extraction must run on a Nitro-capable Linux host with `nitro-cli`.
...
PYTHONPATH=enclave .venv/bin/pytest enclave/tests cli/tests infrastructure/tests
...
Latest local validation results, run on May 15, 2026 with `./scripts/test-local.sh`:

- gateway tests: 15 passed
- Python tests: 41 passed across enclave, CLI, and infrastructure checks
- React build: passed
- Terraform fmt, init, and validate: passed
